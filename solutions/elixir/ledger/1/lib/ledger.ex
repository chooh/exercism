defmodule Ledger do
  ...
  def format_amount(currency, locale, amount_in_cents) do
    number =
      if locale == :en_US do
        decimal =
  ...
  defp format_zz(:usd, :en_US, number) when number >= 0, do: "  $#{number} "
  defp format_zz(:usd, :en_US, number), do: " ($#{number})"
  defp format_zz(:eur, :en_US, number) when number >= 0, do: "  €#{number} "
  defp format_zz(:eur, :en_US, number), do: " (€#{number})"
