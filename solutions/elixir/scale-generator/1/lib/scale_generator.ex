defmodule ScaleGenerator do
  ...
  def step(scale, tonic, step) do
    ...
    |> Enum.drop_while(fn e -> e != tonic end)
    |> Enum.at(
      case step do
        "m" -> 1
        "M" -> 2
  ...
  def flat_chromatic_scale(tonic \\ "C") do
    ...
    Enum.concat(@flat_scale, @flat_scale)
    |> Enum.drop_while(&(&1 != tonic))
    |> Enum.take(13)
  end
  ...
  def scale(tonic, pattern) when tonic in ~w(F bb Eb d g Db),
    do: scale(String.capitalize(tonic), pattern, @flat_scale, [])

  def scale(tonic, pattern), do: scale(String.capitalize(tonic), pattern, @scale, [])

  defp scale(tonic, <<step::utf8, rest::binary>>, scale, list) do
    next_tonic = step(scale, tonic, <<step>>)
    scale(next_tonic, rest, scale, [tonic | list])
  end

  defp scale(tonic, <<>>, _, list), do: Enum.reverse([tonic | list])
end
